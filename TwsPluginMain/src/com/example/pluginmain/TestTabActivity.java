...
public class TestTabActivity extends TabActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tabHost.addTab(tabHost.newTabSpec("First").setIndicator("来自插件一").setContent(intent));

		intent = new Intent("test.xyz1");
		PluginIntentResolver.resolveActivity(intent);// 修正intent
		tabHost.addTab(tabHost.newTabSpec("Second").setIndicator("来自插件二").setContent(intent));
